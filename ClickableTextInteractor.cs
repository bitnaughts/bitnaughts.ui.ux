...
{   
    ...
    public void Initialize(Interactor interactor, OverlayInteractor overlayInteractor, string text, int line, int pos) {
        ...
        // print (text + " " + line + " " + pos);
        this.GetComponent<RectTransform>().localPosition = new Vector2(-10f + (pos - (text.Length-1)/2f) * 50f, -160f + line * -100f);//new Vector2(-50f + (pos - (text.Length-1)/2f) * 37.5f, -83f + line * -75f);
